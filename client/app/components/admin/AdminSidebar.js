    ...
    <aside className="fixed left-0 top-0 h-screen w-64 bg-[#111111] text-white flex flex-col z-50">
      ...
      <div className="px-6 py-8 border-b border-white/10">
        <Link href="/" className="block">
          ...
          <span className="block text-xs text-white/40 tracking-widest uppercase mt-1">
            Admin Console
          </span>
        </Link>
      </div>
        ...
        {NAV.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex items-center gap-3 px-3 py-3 rounded-md text-sm transition-all ${
                isActive
